...
export const authOptions: NextAuthOptions = {
    ...
    providers: [
        CredentialsProvider({
            ...
            async authorize(credentials) {
                ...
                console.log("Success: Authorizing user with role:", user.role);
    ...
    callbacks: {
        async jwt({ token, user }) {
            if (user) {
                token.id = user.id;
                // Combine both: Role from your branch, isFirstLogin from main
        ...
        async session({ session, token }) {
            if (session.user) {
                ...
                // Make both available to your frontend components
